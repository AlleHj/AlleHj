...
class MailAgentScanner:
    ...
    def _process_single_mail(self, msg):
        ...
        # Skapa en säker temporär katalog för detta mail
        temp_dir = Path(tempfile.mkdtemp())

        try:
            attachment_paths = self._save_attachments(msg, temp_dir)
            ...
            try:
                ...
                LOGGER.warning(f"Kunde inte städa upp temp-mapp {temp_dir}: {e}")

        self.hass.add_job(self._notify_update)

    def _save_attachments(self, msg, temp_dir):
        saved_paths = []
        if msg.is_multipart():
